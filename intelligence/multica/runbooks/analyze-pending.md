...
`research plan` 与 `research run` 必须复用同一个实际 cutoff。系统会把较窄的调用方 cutoff 扩展到七天补读窗口，不得在 hydrate 时又换回较窄时间。对于 canonical URL 与 Feed 条目 URL 相同的 RSS 文章，Feed 的 item-scoped 发布时间优先于浏览器页面提取时间；后者可能误取相关推荐卡片日期。
