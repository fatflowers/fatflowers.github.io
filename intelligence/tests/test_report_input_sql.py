...
def test_report_query_excludes_baseline_discovery_undated_and_old():
    ...
    db.executescript("""
        CREATE TABLE items(id, target_id, channel_id, published_at, fetched_at, is_baseline, raw_metadata_json);
        CREATE TABLE analyses(item_id, summary, key_change, why_it_matters, company_impact,
    ...
    for item_id, published, baseline, metadata in [
        ...
        db.execute("INSERT INTO items VALUES (?, 't', 'c', ?, '2026-09-05T06:00:00Z', ?, ?)",
    ...
    results = db.execute(query, ("2026-09-05T00:00:00Z", "2026-09-06T00:00:00Z", 1,
                                 None, None, None, None, None, 500)).fetchall()
